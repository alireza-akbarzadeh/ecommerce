...
import {
  ...
  GetSelectionsListByIdQueryResultApiResult,
  useDeleteAdminGeneralDataSelectionsListByIdAndSelectionsListIdMutation,
...
import useChoicesListGrid from '../../hooks/useChoicesListGrid'
...
const ChoicesList = () => {
  ...
  const actionUrl = `${process.env.NEXT_PUBLIC_GATEWAY}/Admin/GeneralData/SelectionsList/${surveyId}/GetAll`
  const gridRef = useRef<HBDataGridClientRef>(null)
  const [deleteSelectionsList] =
    useDeleteAdminGeneralDataSelectionsListByIdAndSelectionsListIdMutation()

  const [selectedRows, setSelectedRows] = useState<GetAllSelectionsListQueryResult[]>([])
  const [isOpenAddEditDialog, setIsOpenAddEditDialog] = useState<{
    show: boolean
    data?: GetAllSelectionsListQueryResult
  }>({
    show: false,
  })
  ...
  const {
    ...
    refreshGridData,
  } = useChoicesListGrid(gridRef)

  const handleAdd = () => {
    setIsOpenAddEditDialog({ show: true })
  }
  const handleEdit = (props: GetAllSelectionsListQueryResult) => {
  ...
  const handleDelete = useCallback(async () => {
    ...
    for (const [index, id] of ids.entries()) {
      gridLoading(true)
      await deleteSelectionsList({
        'client-name': 'admin',
        'client-version': '1.0.0',
        id: surveyId,
        selectionsListId: String(id),
      })
        .unwrap()
        .then((res: GetSelectionsListByIdQueryResultApiResult) => {
          if (res?.success) {
            openToast({
  ...
  return (
    <Box sx={{ minHeight: 420 }}>
      ...
      <HBDialog
        content={
          <AddEditForm
            selectionsListData={isOpenAddEditDialog?.data}
        ...
        title={formatMessage(surveyMessages.choiceList)}
      />
      <HBDialog
        content={formatMessage(surveyMessages.confirmationMessageDeletingChoices, {
          msg: deleteDialogState?.id ? 1 : selectedRows.length,
        })}
        title={formatMessage(surveyMessages.confirmationTitleDeletingChoices)}
