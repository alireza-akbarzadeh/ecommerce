...
export type AddEditFormCompaniesProps = {
  id: string
  code: string
  title: string
  description?: string
...
export default function AddEditFormStatus({ open, onClose, id, onSave }: AddEditStatusOneProps) {
  ...
  const contentFormProvider = useForm<AddEditFormCompaniesProps>({
    mode: 'all',
  })
  ...
  const handleSubmit = (values: AddEditFormCompaniesProps) => {
    if (!id) {
      addState({
      ...
      <Box sx={{ width: 500 }}>
        <HBForm formProviderProps={contentFormProvider} onSubmit={handleSubmit}>
          ...
          <Grid container spacing={6} direction="row">
            ...
            <Grid item xs={12} md={6}>
              <HBAutocompleteController<AddEditFormCompaniesProps, GetBusinessEntitiesQueryResult>
