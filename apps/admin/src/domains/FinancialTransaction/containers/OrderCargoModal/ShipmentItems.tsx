...
interface ShipmentItemsProps {
  shipmentProducts: ShipmentProductDto[] | null | undefined
  deliveryHour: string
  deliveryDate: string
}
...
const ShipmentItems: FC<ShipmentItemsProps> = ({
  ...
  const createFields = () => {
    return shipmentProducts?.map((product) => {
      return (
        <>
          <Grid item xs={12} sm={2} py={4}>
            ...
            >
              ...
              <Box>{product?.shoppingCartQuantity}</Box>
              <Box>
                <HBIconButton
                  variant="text"
                  icon="trashAlt"
          ...
          <Grid item xs={12} sm={7} py={4}>
            ...
            <ListItem>
              <Stack direction={'column'}>
                {product?.specificAttributes?.map((specificAttribute: SpecificAttributeDto) => (
                  <Typography variant="body2" pb={2}>
                    {`${specificAttribute?.valueTitle}`}
                  </Typography>
                ))}
                {product?.attribute?.map((attribute: AttributeDto) => (
                  <Typography variant="body2" pb={2}>
            ...
            {!!FinancialTransactionMessage.number && (
              <Box mt={6} px={4}>
                <Typography variant="body2">
                  {`${product?.shoppingCartQuantity?.toLocaleString()} ${formatMessage(
                    FinancialTransactionMessage.number,
                  )}`}
                </Typography>
              </Box>
          ...
          <Grid item xs={12} sm={3}>
            <List aria-label="contacts">
              {product?.totalFinalPrice !== product?.totalOriginalPrice && (
                ...
                <ListItemText>
                  <Box display={'flex'}>
                    <Typography pr={1} variant={'body2'}>
                      {`${formatMessage(FinancialTransactionMessage.pickup)}`}
                    </Typography>
                    <Typography variant={'body2'} sx={{ color: 'grey.500' }}>
                      {`${convertDateToPersian(product?.pickupDate!)}`}
                    </Typography>
                  </Box>
                </ListItemText>
              </ListItem>
              <ListItem disablePadding>
                <Box display={'flex'}>
                  ...
                  <Typography variant={'body2'} sx={{ color: 'grey.500' }}>
                    {`${convertDateToPersian(product?.pickupTime!)}`}
                  </Typography>
                </Box>
              </ListItem>
              <ListItem disablePadding>
                <ListItemText>
                  <Box display={'flex'}>
                    ...
                    <Typography variant={'body2'} sx={{ color: 'grey.500' }}>
                      {`${convertDateToPersian(deliveryDate!)}`}
                    </Typography>
                  </Box>
                </ListItemText>
              </ListItem>
              <ListItem disablePadding>
                <ListItemText>
                  <Box display={'flex'}>
                    ...
                    <Typography variant={'body2'} sx={{ color: 'grey.500' }}>
                      {`${convertDateToPersian(deliveryHour!)}`}
                    </Typography>
                  </Box>
                </ListItemText>
              </ListItem>
