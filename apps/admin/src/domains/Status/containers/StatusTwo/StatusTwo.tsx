...
import { useDeleteWorkflowStateMachineByStateMachineIdStateAndStateIdMutation } from '@hasty-bazar/admin-shared/services/workflowApi.generated'
...
import { AddEditFormStatusTwo } from './containers'
import useStatusTwo from './useStatusTwo'
...
const PeriodFilterTwo: FC = ({ data, node, api }: ICellRendererParams) => {
  ...
  const [selectedRows, setSelectedRows] = useState<SelectRowModelTwo[]>([])
  ...
  const [deleteFilterItem] = useDeleteWorkflowStateMachineByStateMachineIdStateAndStateIdMutation()
  ...
  const handleChangedGridActions = (value: number | string, type: 'search' | 'status') => {
    if (type === 'status') {
      ...
      if (value) {
        ...
        gridRef.current!.removeFilter('searchStatusTwo')
      }
    }
  }
  ...
  const handleChangedSelectedRows = (selectedRows: SelectRowModelTwo[]) => {
  ...
  const { actionUrl, columnDefs, autoGroupColumnDef } = useStatusTwo({
    gridRef,
    selectedRows,
    onDelete,
    editId,
  ...
  return (
    <Box height={600} px={10}>
      <HBDataGridClient
        ...
        {...{ columnDefs }}
        masterDetail
        detailCellRenderer={StatusThree}
        detailCellRendererParams={{ stateMachineId: data?.id! }}
        detailRowAutoHeight
        rowHeight={50}
