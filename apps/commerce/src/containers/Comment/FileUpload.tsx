...
import { AddFileStyle, FileCardStyle } from './Comment.style'
import { IFileUpload } from './CommentDialog'
...
const FileUpload: FC<FileUploadProps> = ({ accept, labelText = '' }) => {
  ...
  const handleAddFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    //   return openToast({
    //     message: formatMessage(CommentMessages.uploadFileLimit),
    //     type: 'error',
    //     vertical: 'top',
    //   })
    ...
    fetch(`${process.env.NEXT_PUBLIC_CMS_URL}/Contents/file`, {
      ...
      .then((res) => {
        if (res.success) {
          ...
          openToast({
            message: formatMessage(CommentMessages.fileFormatNotValid),
            type: 'error',
            vertical: 'top',
          })
        }
    ...
    await deleteWebCmsContentsByIdMutation({
      ...
      .then((payload) => {
        if (payload.success) {
          ...
          openToast({
            message: formatMessage(CommentMessages.deleteFileSuccess),
            type: 'success',
            vertical: 'top',
          })
        }
