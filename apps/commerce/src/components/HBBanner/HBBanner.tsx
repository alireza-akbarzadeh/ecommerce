...
interface HBBannerProps extends ContentData {
  closable?: boolean
  type?:
    | 'IMAGE_DEFAULT'
    | 'IMAGE-WITHOUT-TITLE'
    | 'IMAGE_SLIDER_DEFAULT'
    | 'IMAGE_TWO_ROWS'
    | 'IMAGE_ONLY'
    | string
  sx?: SxProps<Theme>
...
const HBBanner: FC<HBBannerProps> = (props) => {
  ...
  const calcBoxWidth = useMemo(() => {
    if (type === 'IMAGE_SLIDER_DEFAULT') return { xs: 172, sm: 268, md: 464 }
    return 464
  }, [type])
  ...
  return (
    <Box
      display={'flex'}
      justifyContent={type === 'IMAGE_SLIDER_DEFAULT' ? 'start' : 'center'}
      alignItems={type === 'IMAGE_SLIDER_DEFAULT' ? { xs: 'start', md: 'center' } : 'center'}
      sx={{
        height: calcHeight,
        px: { xs: 2, sm: 'unset' },
        pt: type === 'IMAGE_SLIDER_DEFAULT' ? { xs: 2, sm: 4, md: 'unset' } : 'unset',
        position: 'relative',
        borderRadius: 4,
        overflow: 'hidden',
        ...sx,
    ...
    >
      {!!imageUrl && (
        ...
        >
          <Box
            src={process.env.NEXT_PUBLIC_CDN! + imageUrl!}
            width="100%"
            height="100%"
            component="img"
            sx={{
      ...
      {closable && (
        ...
        >
          <HBIcon
            type="times"
            // size="small"
            sx={{ color: 'grey.500', fontSize: 16, display: 'flex' }}
          />
        </Box>
      )}
      {type !== 'IMAGE_ONLY' && (!!title || !!description) && (
        <Stack
          alignItems={type === 'IMAGE_SLIDER_DEFAULT' ? '' : 'center'}
          ...
          sx={{
            ...
            opacity: type === 'IMAGE_SLIDER_DEFAULT' ? 0.8 : 'unset',
            flexDirection: calcDirectionBox,
            zIndex: 99,
            ml: type === 'IMAGE_SLIDER_DEFAULT' ? { md: 3.5 } : 0,
          }}
        >
          ...
          {metadata?.hasButton && (
            <Box
              display={type === 'IMAGE_SLIDER_DEFAULT' ? { xs: 'none', sm: 'flex' } : 'flex'}
              justifyContent="end"
              mt={type !== 'IMAGE-WITHOUT-TITLE' ? 6 : 0}
            >
              <HBButton
                onClick={(event) => {
...
HBBanner.defaultProps = { type: 'IMAGE_DEFAULT' }

export default HBBanner
