import HBSelectController from '@hasty-bazar/admin-shared/containers/HBSelectController'
import { HBSwitchController } from '@hasty-bazar/admin-shared/containers/HBSwitchController'
import HBTextFieldController from '@hasty-bazar/admin-shared/containers/HBTextFieldController'
import HBTinyEditorController from '@hasty-bazar/admin-shared/containers/HBTinyEditorController'
import { BusinessTypeEnums } from '@hasty-bazar/admin-shared/core/enums'
import phrasesMessages from '@hasty-bazar/admin-shared/core/translations/phrases.messages'
import validationsMessages from '@hasty-bazar/admin-shared/core/translations/validations.messages'
import { usePutAdminCmsContentsByIdMutation } from './cmsApi.generated.enhanced'
import { usePutAdminCatalogCategoriesByIdCategoryDefaultImageMutation } from '@hasty-bazar/admin-shared/services/catalogApi.generated'
import { GetContentsByEntityTypeQueryResult } from '@hasty-bazar/admin-shared/services/cmsApi.generated'
import { useGetAdminGeneralDataBusinessTypeValueGetAllValuesByBusinessTypeIdQuery } from '@hasty-bazar/admin-shared/services/generalDataApi.generated'
import { errorsToString } from '@hasty-bazar/admin-shared/utils'
import {
  HBButton,
  HBClassesType,
  HBForm,
  HBFormItemTextField,
  HBIcon,
  HBIconButton,
  openToast,
} from '@hasty-bazar/core'
import { Box, FormHelperText, Grid, Typography } from '@mui/material'
import { ChangeEvent, useEffect, useState } from 'react'
import { useForm } from 'react-hook-form'
import { useIntl } from 'react-intl'
import HBFormItemColorPicker from './HBFormItemColorPicker'
import HBContentUploaderFormMessages from '../HBContentUploader.message'

type HBPageClassnames = 'modalContainer' | 'modalHeader' | 'modalSubTitle' | 'hasButton'

export type updateImagesInputType = {
  name: string
  title: string
  description: string
  hasButton: boolean
  backgroundColor: string
  fontColor: string
  url: string
  button: {
    backgroundColor: string
    fontColor: string
    link: string
    name: string
  }
}

const classes: HBClassesType<HBPageClassnames> = {
  modalContainer: ({ spacing }) => ({
    position: 'absolute',
    top: '50%',
    left: '50%',
    transform: 'translate(-50%, -50%)',
    width: '80%',
    backgroundColor: 'background.paper',
    borderRadius: spacing(1),
    p: 6,
  }),
  modalHeader: {
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  modalSubTitle: ({ palette }) => ({
    color: palette.grey[500],
    my: 4,
  }),
  hasButton: {
    display: 'flex',
    alignItems: 'center',
    gap: 4,
  },
}

export type PictureSettingModalProps = {
  handleCloseSettings: () => void
  data: GetContentsByEntityTypeQueryResult
  refetch: () => void
}

export default function PictureSettingModal(props: PictureSettingModalProps) {
  const [hasButton, setHasButton] = useState<boolean>(false)
  const [link, setLink] = useState<string>('')
  const [isFirstLoad, setIsFirstLoad] = useState<boolean>(true)
  const [updateImage] = usePutAdminCatalogCategoriesByIdCategoryDefaultImageMutation()
  const formProviderProps = useForm<updateImagesInputType>({
    mode: 'all',
  })
  const {
    handleCloseSettings,
    data: { metaData = '', id, name, title, description, ...otherProps },
    refetch,
  } = props
  const { formatMessage } = useIntl()
  const [updateImages, { error }] = usePutAdminCmsContentsByIdMutation()
  let formData = {}
  try {
    formData = JSON.parse(metaData!)
  } catch (error) {
    formData = {}
  }

  const { data: { data: { items: businessTypeData = [] } = {} } = {} } =
    useGetAdminGeneralDataBusinessTypeValueGetAllValuesByBusinessTypeIdQuery({
      'client-name': 'generalData',
      'client-version': '0',
      businessTypeCode: BusinessTypeEnums.LinkNavigationType,
      pageSize: 1000,
    })

  const { data: { data: { items: linkNavigationWebData = [] } = {} } = {} } =
    useGetAdminGeneralDataBusinessTypeValueGetAllValuesByBusinessTypeIdQuery({
      'client-name': 'generalData',
      'client-version': '0',
      businessTypeCode: BusinessTypeEnums.LinkNavigationWeb,
      pageSize: 1000,
    })

  useEffect(() => {
    if (error) {
      openToast({
        message: errorsToString(error),
        type: 'error',
      })
    }
  }, [error])

  useEffect(() => {
    if (isFirstLoad) {
      setHasButton((formData as any)?.hasButton)
      setLink((formData as any)?.button.link)
      formProviderProps.reset({
        ...formData,
        name: name ?? '',
        description: description ?? '',
        title: title ?? '',
      })
      setIsFirstLoad(false)
    }
  }, [formData])

  const onChangeSwitch = (event: ChangeEvent<HTMLInputElement>, checked: boolean) => {
    setHasButton(checked)
  }

  const onChangeLink = (value: string) => {
    setLink(value)
  }

  const updateImageHandler = (value: string) => {
    if (props.data?.factor === 'main') {
      updateImage({
        'client-name': 'vendor',
        'client-version': '1.0.0',
        id: props.data?.entityId!,
        setCategoryDefaultImageModel: {
          defaultImage: props?.data?.value,
          imageMetaData: value,
        },
      })
    }
  }
  const getLengthHandler = (value: string) => {
    return value
      ?.replace(/(<([^>]+)>)/gi, '')
      ?.replace(/&nbsp;/g, '')
      ?.trim().length
  }

  return (
    <HBForm<updateImagesInputType>
      formProviderProps={formProviderProps}
      defaultValues={{
        ...formData,
        name: name ?? '',
        description: description ?? '',
        title: title ?? '',
      }}
      onSubmit={({ name, title, description, ...value }) => {
        updateImages({
          'client-name': 'hasty-bazar-admin',
          'client-version': '1.0.0',
          id: String(id!),
          //@ts-ignore//
          updateContentModel: {
            ...otherProps!,
            metadata: JSON.stringify({ ...value }),
            name,
            title,
            description,
          },
        }).then(() => {
          refetch()
          handleCloseSettings()
          updateImageHandler(JSON.stringify({ ...value }))
        })
      }}
    >
      <Box sx={classes.modalContainer}>
        <Box sx={classes.modalHeader}>
          <Typography variant="h5">
            {formatMessage(HBContentUploaderFormMessages.pictureModalMainTitle)}
          </Typography>
          <HBIconButton onClick={handleCloseSettings} variant="text" icon="times" />
        </Box>
        <Typography sx={classes.modalSubTitle}>
          {formatMessage(HBContentUploaderFormMessages.pictureModalTitle)}
        </Typography>

        <Grid container spacing={6}>
          <Grid container item xs={12} spacing={6}>
            <Grid item xs={12} sm={6}>
              <HBTextFieldController
                label={formatMessage(HBContentUploaderFormMessages.pictureModalName)}
                name="name"
                InputLabelProps={{ required: true }}
                formRules={{
                  maxLength: {
                    value: 80,
                    message: formatMessage(
                      HBContentUploaderFormMessages.theNumberOfLettersMustBeLessThan80,
                    ),
                  },
                  validate: (value) =>
                    !!value.trim() ||
                    `${formatMessage(validationsMessages.isRequired, {
                      msg: '',
                    })}`,
                  required: {
                    value: true,
                    message: `${formatMessage(validationsMessages.isRequired, {
                      msg: formatMessage(HBContentUploaderFormMessages.pictureModalName),
                    })}`,
                  },
                }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBFormItemTextField
                label={formatMessage(HBContentUploaderFormMessages.title)}
                formName="title"
                inputProps={{ maxLength: 150 }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBTextFieldController
                label={formatMessage(HBContentUploaderFormMessages.webRedirectLink)}
                name="url"
                formRules={{
                  required: {
                    value: true,
                    message: `${formatMessage(validationsMessages.isRequired, {
                      msg: formatMessage(HBContentUploaderFormMessages.webRedirectLink),
                    })}`,
                  },
                }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBSelectController
                sx={{
                  width: {
                    md: '100%',
                    xs: '100%',
                  },
                  minWidth: 100,
                }}
                label={formatMessage(HBContentUploaderFormMessages.webLinkNavigationMethod)}
                name={'webLinkNavigationType'}
                menuItem={
                  linkNavigationWebData?.map((item) => ({
                    title: String(item.title),
                    value: item.id || 0,
                  })) || []
                }
                disabled={!formProviderProps?.watch('url')}
                formRules={{ required: false }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBTextFieldController
                label={formatMessage(HBContentUploaderFormMessages.mobileRedirectLink)}
                name="mobileUrl"
                formRules={{
                  required: {
                    value: true,
                    message: `${formatMessage(validationsMessages.isRequired, {
                      msg: formatMessage(HBContentUploaderFormMessages.mobileRedirectLink),
                    })}`,
                  },
                }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBSelectController
                sx={{
                  width: {
                    md: '100%',
                    xs: '100%',
                  },
                  minWidth: 100,
                }}
                label={`${formatMessage(
                  HBContentUploaderFormMessages.mobileLinkNavigationMethod,
                )}*`}
                name={'linkNavigationType'}
                menuItem={
                  businessTypeData?.map((item) => ({
                    title: String(item.title),
                    value: item.id || 0,
                  })) || []
                }
                formRules={{ required: false }}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBFormItemColorPicker
                label={formatMessage(HBContentUploaderFormMessages.backgroundColor)}
                formName="backgroundColor"
                saveButtonLabel={formatMessage(phrasesMessages.save)}
                cancelButtonLabel={formatMessage(phrasesMessages.cancel)}
              />
            </Grid>
            <Grid item xs={12} sm={6}>
              <HBFormItemColorPicker
                label={formatMessage(HBContentUploaderFormMessages.fontColor)}
                formName="fontColor"
                saveButtonLabel={formatMessage(phrasesMessages.save)}
                cancelButtonLabel={formatMessage(phrasesMessages.cancel)}
              />
            </Grid>
            <Grid item xs={12}>
              <HBTinyEditorController
                name={`description`}
                init={{ max_height: 200 }}
                formRules={{
                  validate: (value) => {
                    return !(getLengthHandler(value) > 500)
                  },
                }}
              />
              {getLengthHandler(formProviderProps?.watch('description')) > 500 ? (
                <FormHelperText
                  sx={(theme) => {
                    return {
                      color: 'error.main',
                      fontSize: theme.typography.caption.fontSize,
                      display: 'flex',
                      alignItems: 'center',
                      gap: 2,
                    }
                  }}
                >
                  <HBIcon size="small" type="infoCircle" />
                  {formatMessage(
                    HBContentUploaderFormMessages.theNumberOfDescriptionMustBeLessThan500,
                  )}
                </FormHelperText>
              ) : null}
            </Grid>
          </Grid>

          <Grid container item xs={12} spacing={6}>
            <Grid item xs={12} sm={6}>
              <Box sx={classes.hasButton}>
                <Typography>{formatMessage(HBContentUploaderFormMessages.hasButton)}</Typography>
                <HBSwitchController name="hasButton" disabled={false} onChange={onChangeSwitch} />
              </Box>
            </Grid>

            <Grid item xs={12} sm={6}>
              <HBFormItemTextField
                label={formatMessage(HBContentUploaderFormMessages.buttonName)}
                formName="button.name"
                disabled={!hasButton}
              />
            </Grid>

            <Grid item xs={12} sm={6}>
              <HBFormItemTextField
                label={formatMessage(HBContentUploaderFormMessages.buttonLink)}
                formName="button.link"
                disabled={!hasButton}
                onChange={(e) => onChangeLink(e.target.value)}
                rules={{ required: false }}
              />
            </Grid>

            <Grid item xs={12} sm={6}>
              <HBSelectController
                sx={{
                  width: {
                    md: '100%',
                    xs: '100%',
                  },
                  minWidth: 100,
                }}
                label={formatMessage(HBContentUploaderFormMessages.linkNavigationType)}
                name={'button.linkNavigationType'}
                menuItem={
                  businessTypeData?.map((item) => ({
                    title: String(item.title),
                    value: item.id || 0,
                  })) || []
                }
                formRules={{ required: false }}
                disabled={!hasButton || !link}
              />
            </Grid>

            <Grid item xs={12} sm={6}>
              <HBFormItemColorPicker
                label={formatMessage(HBContentUploaderFormMessages.buttonBackgroundColor)}
                formName="button.backgroundColor"
                saveButtonLabel={formatMessage(phrasesMessages.save)}
                cancelButtonLabel={formatMessage(phrasesMessages.cancel)}
                disabled={!hasButton}
              />
            </Grid>

            <Grid item xs={12} sm={6}>
              <HBFormItemColorPicker
                label={formatMessage(HBContentUploaderFormMessages.buttonFontColor)}
                formName="button.fontColor"
                saveButtonLabel={formatMessage(phrasesMessages.save)}
                cancelButtonLabel={formatMessage(phrasesMessages.cancel)}
                disabled={!hasButton}
              />
            </Grid>
          </Grid>
        </Grid>

        <Box display="flex" justifyContent="space-between" mt={8}>
          <HBButton variant="outlined" sx={{ width: 120 }} onClick={() => handleCloseSettings()}>
            {formatMessage(phrasesMessages.back)}
          </HBButton>
          <HBButton type="submit" sx={{ width: 120 }}>
            {formatMessage(phrasesMessages.save)}
          </HBButton>
        </Box>
      </Box>
    </HBForm>
  )
}
